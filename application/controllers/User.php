...
class User extends CI_Controller {
	...
	function jadwal(){
		...
		$data['jadwal']= $this->m_user->getJadwalAuditor('s_tmp.tb_jadwal','s_mst.tb_user',$where)->result();
	...
	function data_lk($auditie){
		...
		$id_dept       = $auditie;
		$dept          = $this->session->userdata('dept');
        $where         = array('s_mst.tb_audit.kd_dept_audit' => $id_dept, 'otorisasi' => 'SUDAH');
        $where2        = array('id_dept' => $id_dept);
        $data['dept']  = $this->m_user->getWhere('s_mst.tb_dept', $where2)->result();
		$data['area']  = $data['dept'][0]->kat_dept;
        $data['audit'] = $this->m_user->getAllAudit('s_mst.tb_audit', 's_mst.tb_dept', 's_mst.tb_aspek', 's_mst.tb_par_temuan', $where)->result();
	...
	function ranking(){
		...
		$data['ranking'] = $this->m_user->getRankingData('s_mst.tb_ranking', 's_mst.tb_dept', $where)->result();
